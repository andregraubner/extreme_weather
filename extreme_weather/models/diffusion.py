"""
In large part taken from github.com/lucidrains/med-seg-diff-pytorch,
but added support for non-square spatial dimensions in tensors
"""
...
class Conditioning(nn.Module):
    def __init__(self, fmap_size: Tuple[int, int], dim):
        super().__init__()
        self.ff_parser_attn_map = nn.Parameter(torch.ones(dim, fmap_size[0], fmap_size[1]))
...
class Unet(nn.Module):
    def __init__(
        ...
        for ind, ((dim_in, dim_out), full_attn) in enumerate(zip(in_out, full_self_attn)):
            ...
            if not is_last:
                curr_fmap_size = tuple(i // 2 for i in curr_fmap_size)
...
class MedSegDiff(nn.Module):
    ...
    def sample(self, cond_img):
        ...
        return sample_fn((batch_size, mask_channels, image_size[0], image_size[1]), cond_img)
